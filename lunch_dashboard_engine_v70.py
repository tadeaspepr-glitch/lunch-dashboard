...
def info_screen_config(args) -> dict:
    ...
    until_h, until_m = (args.menu_until.split(":") + ["0"])[:2]
    return {
        ...
        "lat": args.lat,
        "lon": args.lon,
        "menuSeconds": max(5, args.menu_seconds),
        "infoSeconds": max(5, args.info_seconds),
        "menuUntil": int(until_h) * 60 + int(until_m),
    }
...
(function () {
  ...
  function menuAllowed() {
    var now = new Date();
    if (!MENU_DATE || MENU_DATE !== isoDate(now)) { return false; }
    return now.getHours() * 60 + now.getMinutes() < CFG.menuUntil;
  }
  function show(name) {
    current = name;
  ...
  function cycle() {
    var next;
    if (!menuAllowed()) { next = "info"; }
    else { next = current === "menu" ? "info" : "menu"; }
    show(next);
    setTimeout(cycle, (next === "menu" ? CFG.menuSeconds : CFG.infoSeconds) * 1000);
  }
  ...
  if (menuAllowed()) { show("menu"); setTimeout(cycle, CFG.menuSeconds * 1000); }
  else { show("info"); setTimeout(cycle, CFG.infoSeconds * 1000); }
})();
"""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Polední menu dashboard v7.0")
    ...
    ap.add_argument("--menu-seconds", type=int, default=45, help="Jak dlouho svítí obrazovka s menu")
    ap.add_argument("--info-seconds", type=int, default=20, help="Jak dlouho svítí obrazovka s informacemi")
    ap.add_argument("--menu-until", default="14:30", help="Po tomto čase už TV ukazuje jen informační obrazovku")
